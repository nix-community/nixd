...
Test that Pack action works correctly for 'bar' prefix when multiple independent prefixes exist.
...
```nix file:///pack-attrs-multiple-prefixes-bar.nix
{ foo.a = 1; foo.b = 2; bar.c = 3; bar.d = 4; }
```

<-- textDocument/codeAction(2) - Test 'bar' prefix
...
```json
{
   ...
   "params":{
      "textDocument":{
         "uri":"file:///pack-attrs-multiple-prefixes-bar.nix"
      },
      "range":{
         "start":{
            "line": 0,
